...
#[derive(Debug)]
pub enum KeyEvent {
    Press(xcb::x::KeyPressEvent),
    Release(xcb::x::KeyReleaseEvent),
}
...
extern "C" fn forward_event_callback(
    ...
    let event = unsafe {
        if pressed {
            KeyEvent::Press(xcb::x::KeyPressEvent::from_raw(ptr as _))
        } else {
            KeyEvent::Release(xcb::x::KeyReleaseEvent::from_raw(ptr as _))
        }
    };
    let ime = unsafe { ime_from_user_data(user_data) };
    let win = unsafe { Window::new(ime.pos_req.win) };
...
type KeyPressCB = dyn for<'a> FnMut(Window, &'a KeyEvent);
...
impl ImeClient {
    ...
    where
        F: for<'a> FnMut(Window, &'a KeyEvent) + 'static,
    {
